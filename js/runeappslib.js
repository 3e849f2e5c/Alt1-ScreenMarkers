...
trackables=["totlv","qpxam"];//create array with all trackables
...
function reporterror(message,file,line,col,error){
	...
	if(reportederrors.length<2 && false){//disable this feature for now
		...
		clog("Automated error report sent");}
	else{
		clog("Error not reported.")
	}
}
...
function promptbox(title, options, buttons, func) {
	...
	var framefunc;
	if (options.style == "nis") { framefunc = promptbox_nis; }
...
function promptbox2(options,buttons) {
	...
	var closeclicked = function () {
		if (options.onclose() !== false) { close(); }
	}
	...
	if (box.insertfunc) {
		...
		top.document.body.appendChild(box.root);
		box.root.classList.remove("measuring");
...
function promptbox_nis(options) {
	var els = { root: null, title: null, closebutton: null, contentbox: null, insertfunc: null, closefunc: null };
	els.root = elcreate("div", options.v2 ? "pb2 pb2_nis" : "nisborder promptbox promptbox_nis", null, null, [
...
function smartxptolvl(xp,skill,capped){
	...
	//apply cap
	if(capped){
		if(skill=="inv" || skill=="dun"){lvl=Math.min(120,lvl);}
...
function balloonbox(text,box,buttons,closefunction){
  ...
  if(buttons){
    ...
    for(c in buttons){//[0text,1fun[,type]]
      ...
      if(buttons[c][2]=="cancel"&&!closefunction){closefunction=buttons[c][1];}}//function with name cancel is also used for canceling if cancelcallback is undefined
    str+="</div>";}
...
function newdraghandler(movefunc,endfunc){
	if(draghandler){
		if(draghandler.end){draghandler.end(mouseloc);}
		draghandler=false;}//clear old draghandler
	draghandler={move:movefunc,end:endfunc,sx:mouseloc.x,sy:mouseloc.y};
	mouseloc.sx=mouseloc.x;
	mouseloc.sy=mouseloc.y;
	return mouseloc;
}

function globalmousemove(e){
	mouseloc.dx=e.clientX-mouseloc.x;
	mouseloc.dy=e.clientY-mouseloc.y;
	mouseloc.x=e.clientX;
	mouseloc.y=e.clientY;
	if(draghandler){draghandler.move(mouseloc);}
}
...
function globalmouseup(e){
	...
	if(draghandler){
		if(draghandler.end){draghandler.end(mouseloc);}
		else if(draghandler.move){draghandler.move(mouseloc,true);}
		draghandler=false;}
}
...
function startbarslide(bar, cb, cbdone) {
	...
	newdraghandler(callback, function (a) { callback(a); if (cbdone) { cbdone(a); } });
}
...
function overrideselect(frame) {
	...
	overrideselected=function(e){
		...
		var el = modeldropdown(func, names, values);
...
function modeldropdown(func, names, values, selected) {
	var a,b,c,str,el;
...
function makegraph(el, vals, times, opts) {
	...
	if ((vmax - vmin) / a > 1000) { clog("wtf error shouldnt happen"); return; }
...
function reportimagebug(image, reportname, donecb) {
	...
	uploaded = function (t) {
		...
		if (!id) { clog("Upload failed, error: " + r); return; }
		dlpagepost("/submitbug.php",
			{ body: "-- automated image report --\n-- " + reportname + " --\nhttp://" + document.location.host + "/i/" + id[1] },
			function (t) { if (donecb) { donecb(); } },
			function () { clog("error sending auto report"); });
	}

	imgdata = image.toDataURL("image/png");
	dlpagepost("/i/upload.php", { img: imgdata }, uploaded, clog("Failed to upload bug image"));
}
...
function validateclone(obj, str) {
	...
	catch (b) { clog("Failed to validate object, error: " + b); return false; }
	if (typeof a != "object") { clog("Failed to validate object, error: " + a); return false; }
	return a.val;
}
...
function pagepopup(pagename,w,h){
	return a1lib.openpopup("/apps/alt1/help/"+pagename,w,h);
}
...
function bindAutoComplete(el, func) {
	...
	scope.selectSuggestion = function (index) {
		...
		index = Math.max(0, Math.min(scope.suggests.length - 1, index));
		scope.selectindex = index;
		var els = scope.optel.children;
		for (var a = 0; a < els.length; a++) { els[a].classList.remove("selected"); }
		if (els[scope.selectindex]) {
			els[scope.selectindex].classList.add("selected");
			els[scope.selectindex].focus();
		}
		var selstart = el.selectionStart;
		scope.words[scope.wordindex] = scope.suggests[scope.selectindex].t;
		var offset = 0;
		for (var a = 0; a < scope.wordindex; a++) { offset += scope.words[a].length + 1; }
		var selend = offset + scope.words[scope.wordindex].length;
	...
	scope.insertSuggestion = function (str) {
		...
		if (!str) { str = scope.suggests[scope.selectindex].t; }
		scope.words[scope.wordindex] = str;
		el.value = scope.words.join(" ");
		scope.setSuggests(false);
	}

	el.addEventListener("input", scope.obtainSuggests);

	el.addEventListener("blur", function (e) { setTimeout(scope.setSuggests.bind(null, null), 100); });
	...
	el.addEventListener("keydown", function (e) {
		...
		if (e.keyCode ==  9 && scope.suggests && scope.suggests.length > 0) { scope.insertSuggestion(); scope.setSuggests(null); e.preventDefault(); }
		if (e.keyCode == 13 && scope.suggests && scope.suggests.length > 0) { scope.insertSuggestion(); scope.setSuggests(null); e.preventDefault(); e.stopImmediatePropagation(); }
...
Object.defineProperty(Function.prototype, "b", {
	value: function () {
		...
		return function () { return me.apply(null, args); };
	}
});
